...
function createRole(){
  ...
  const permissions = {inventory:true, clients:true, storage:true, supplies:true, outbound:true, fbs:true, dds:true, stocktake:true, warehouses:true, consumables:true, returns:true, employees:true, 'portal-supplies':false};
...
function renderEmployeesTab(){
  ...
  if(currentEmployeesView==='roles'){
    const permLabels = [['inventory','Остатки'],['clients','Клиенты'],['storage','Хранение'],['supplies','Плановые поставки'],['outbound','Поставка на склад'],['fbs','Заказы FBS'],['dds','ДДС'],['stocktake','Инвентаризация'],['warehouses','Склады'],['consumables','Расходники'],['returns','Возвраты'],['employees','Сотрудники']];
    body.innerHTML = `
